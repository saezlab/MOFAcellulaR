...
multiview_dat <- MOFAcellulaR::filt_profiles(pb_dat = pb_obj,
                          cts = c("Fib","CM", "Endo"),
                          ncells = 0,
                          counts_col = "cell_counts", # This refers to the column name in testcoldata where the number of cells per profile was stored
                          ct_col = "cell_type") %>%
  ...
  MOFAcellulaR::filt_gex_byhvg(pb_dat_list = .,
                               prior_hvg = NULL,
                               var.threshold = 0) %>%
  MOFAcellulaR::filt_gex_bybckgrnd(pb_dat_list = ct_list,
                                            prior_mrks = prior_hvg_test) %>%
  MOFAcellulaR::pb_dat2MOFA(pb_dat_list = ct_list)
...
```{r}
model@cache$variance_explained$r2_per_factor$single_group["Factor1",,drop = F]
```
